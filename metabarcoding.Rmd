...
This is to make our reads comparable between sample dates. We could use the standard rarefaction method, because our MALV will not be lost as they are in higher abundance. Currently I don't use the rarefied table at all, as for the relative abundance plots I simply use the relative of the total reads. If we do statistics we would need to use the rarefied or CLR converted one to make it comparable. For the compositional data plots I tried both variants, the overall pattern is the same but if we look in detail at the MALV groups we loose some with the rarefied function, therefore I use mostly the afore mentioned full dataset, but as I said it makes not difference in the conclusion as the pattern is the same.

```{r}
minimum_reads <- min(rowSums(data_otu))
data_otu_raref <- vegan::rrarefy(data_otu, minimum_reads)
...
Using bray curtis distance on rarefied OTU table.

```{r}
data_meta %>% glimpse()
nrow(data_meta)
nrow(data_otu_raref)
nrow(data_otu_raref[which(is.na(data_meta$watermass)), ])
# Thats for the whole composition
(r <-
    vegan::adonis2(
        data_otu_raref[which(!is.na(data_meta$watermass)), ] ~ seasons * watermass + year,
        data = data_meta %>% drop_na(watermass),
        method = "bray",
        perm = 100
    )
)

data_malvs_drop <- data_malvs %>%
    drop_na(watermass) %>%
    mutate(
        year = year(date),
    )
...
(r_m <-
    vegan::adonis2(
        data_malvs_drop$value_p ~ seasons * watermass + year,
        data = data_malvs_drop,
        perm = 1000
    )
)

broom::tidy(r_m) %>% knitr::kable()
```
